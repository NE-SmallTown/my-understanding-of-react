...
export function scheduleUpdateOnFiber(
  ...
  if (expirationTime === Sync) {
    if (workPhase === LegacyUnbatchedPhase) {
      ...
      let callback = renderRootSync(root, Sync);
      while (callback !== null) {
...
function runRootCallback(root, expirationTime, __c, isSync) {
  const prevCallbackNode = root.callbackNode;
  let continuation = null;
  try {
    // continuation 还可能是 commitRoot
    if (__c === null || __c === renderRootAsync || __c === renderRootSync) {
      continuation = isSync ? renderRootSync(root, expirationTime) : renderRootAsync(root, expirationTime);
    } else {
      continuation = __c(isSync);
    }

    if (continuation !== null) {
      return runRootCallback.bind(null, root, expirationTime, continuation);
    } else {
      return null;
    }
  } finally {
...
export function flushRoot(root: FiberRoot, expirationTime: ExpirationTime) {
  ...
  scheduleCallback(
    ImmediatePriority,
    renderRootSync.bind(null, root, expirationTime),
  );
  flushImmediateQueue();
}
...
function flushPendingDiscreteUpdates() {
  if (rootsWithPendingDiscreteUpdates !== null) {
    ...
    roots.forEach((expirationTime, root) => {
      scheduleCallback(
        ImmediatePriority,
        (isSync) => isSync ? renderRootSync(root, expirationTime) : renderRootAsync(root, expirationTime),
      );
    });
  ...
  if (workInProgress !== null) {
    ...
    if (expirationTime !== Sync) {
      ...
      if (currentTime < expirationTime) {
        ...
        return renderRootSync.bind(null, root, currentTime);
      }
    }
    ...
    if (workInProgress !== null) {
      ...
      return renderRootSync.bind(null, root, expirationTime);
    }
  }
  ...
  switch (workInProgressRootExitStatus) {
    ...
    case RootErrored: {
      ...
      if (root.lastPendingTime < expirationTime) {
        ...
        return renderRootSync.bind(null, root, lastPendingTime);
      }
...
function renderRootAsync(
  ...
  if (workInProgress !== null) {
    ...
    if (workInProgress !== null) {
      ...
      return renderRootAsync.bind(null, root, expirationTime);
    }
  }
  ...
  switch (workInProgressRootExitStatus) {
    ...
    case RootErrored: {
      ...
      if (root.lastPendingTime < expirationTime) {
        ...
        // at the lower priority to see if it fixes it. Use a continuation to
        // maintain the existing priority and position in the queue.
        return renderRootAsync.bind(null, root, lastPendingTime);
      }
      ...
      scheduleCallback(
        ImmediatePriority,
        renderRootAsync.bind(null, root, expirationTime),
      );
      return null;
    }
    case RootSuspended: {
